function [spectrum]=impaint2(a,lamda,k00,ke,phi,illumSteps) %这里主要是sim了
...
for ii = 1:length(ke)
    ...
    theta_h = phi(ii) : pi/illumSteps(ii) : pi.*(illumSteps(ii)-1)/illumSteps(ii)+phi(ii); %(第一圈是0 120 240 第二圈就从60开始为60 180 300)
    theta = theta_h;
    k=1;
