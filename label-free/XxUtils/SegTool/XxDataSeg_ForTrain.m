...
for i = 1:num_seg
    ...
    count_rand = 0;
    while (y1<1 || y2>r || x1<1 || x2>r) %单图超分辨把1改成2
        p = randi(l_list,1);
        y1 = point_list(p, 1) - halfy + 1;
        y2 = point_list(p, 1) + halfy;
        x1 = point_list(p, 2) - halfx + 1;
        ...
        if count_rand > 1e8, break; end
    end

    if count_rand > 1e6, break; end
    if RotFlag >= 1 % if random rotate
        ...
        patch_gt = imrotate(gt(sr_ratio*(x1-1)+1:sr_ratio*x2,sr_ratio*(y1-1)+1:sr_ratio*y2,1),degree,'bilinear','crop');
        ...
        patch_gt = gt(sr_ratio*(x1-1)+1:sr_ratio*x2,sr_ratio*(y1-1)+1:sr_ratio*y2,1);
...
%         subplot(1,2,2), imagesc(patch_gt(:,:,1)); axis image, axis off;
    ...
    if RotFlag >= 1
        ...
        tgt = imrotate(gt(sr_ratio*(x1-1)+1:sr_ratio*x2,sr_ratio*(y1-1)+1:sr_ratio*y2,:),degree,'bilinear','crop');
        if isempty(data_seg)
        ...
        if isempty(gt_seg)
            gt_seg = gt(sr_ratio*(x1-1)+1:sr_ratio*x2,sr_ratio*(y1-1)+1:sr_ratio*y2,:);
        else
            h_gt = size(gt, 3);
            gt_seg(:,:,end+1:end+h_gt) = gt(sr_ratio*(x1-1)+1:sr_ratio*x2,sr_ratio*(y1-1)+1:sr_ratio*y2,:);
        end
    end
end
